...
\begin{enumerate}
  \item \deriv
    ...
    {$\_\_$\smallphongroup{L\\N} (C) $\;\big]_\omega$}
  \item same as 1. but with composition boundary instead of word boundary
  \item \deriv
    {/\schwaMobi/}    % E (schwa mobile)
    {/\schwaCons/}    % ə (schwa constans / realization)
    {$\_\_$\smallphongroup{L\\N} (C) + CX}
  \item \deriv
    {/\schwaMobi/}    % E (schwa mobile)
    {/\schwaCons/}    % ə (schwa constans / realization)
    {$\_\_$\smallphongroup{L\\N} (C) + \schwaMobi C(C)}
\end{enumerate}
...
\deriv
  {/\schwaMobi/}    % E (schwa mobile)
  {\O}              % deleted / empty
  {\schwaCons \smallphongroup{L\\N} (C) + $\_\_$ (CC)}%
\footnote{\Textcite{issatschenko1974} actually states the
  following rule:
  \deriv
    {/\schwaMobi/}    % E (schwa mobile)
    {\O}              % deleted / empty
    {\schwaCons \smallphongroup{L\\N} (C) $\_\_$ + \schwaMobi (CC)}\\
...
\begin{structure}{\ex{Handelns} \ti{[hand@lns]}}
  \drawCV{5}
  \C{h}
  \V{a}
  \C{n}
...
sites and \gls{IG}, which can be observed in \ex{Handlung}:

\begin{structure}{\ex{Handlung} \ti{[handlUN]}}
  \drawCV{5}
  \C{h}
  \V{a}
  \C{n}
...
isn't governed, making the word either ungrammatical or forcing
...
by Proper Government\marknote{\emph{Proper Gov.} vorher eingeführt?} but by \gls{IG}, which in turn frees $V_4$ from
its governing duties to $V_3$; it governs $V_2$\footnotemark.
\footnotetext{The \SGP-rule that \tr{governing rules may not cross each other}
is \tr{abgeschafft in} \CVCV.}

While Government and Licensing are dynamic computations and not stored in the lexicon,
...
\item The lexicon stores not static instances of \gls{IG} that are
  ...
  This analysis is analogous to vowel length alternations (\cref{subsubsec:bg:long vowels})
  or the distribution of \ti{[N]}/\ti{[Ng]} (\cref{subsec:phenomen:ng}):
  If these constructions fail to be licensed, there has to be some reaction to save
  grammaticality, in these cases shortening the vowel or not realizing the \ti{[g]}, respectively.

\item Another option to ensure well-formedness in these
  ...
  \footnotetext{Keep in mind that while \gls{IG} needs to be licensed,
  it itself establishes a governing relationship \tr{over/on the enclosed Nucleus}.}
\end{enumerate}
%
If both assumptions hold, \ex{Handlung}
...
Vllt ist bei der Affigierung (?) aber auch einfach mehr Parametrisierung möglich
