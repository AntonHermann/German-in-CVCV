...
\footnote{
  ...
  \TODO{maybe move detailled discussion of indicators for language-specific
  vowel-headedness to \emph{Params -- Long Vowel Headedness}}
}
...
\ex{Brett} or \ex{Vergleich} where there is an empty Nucleus either at the word start or
...
\begin{structure}{\ex{Brett} \ti{[bKEt]}}
...
\begin{structure}{\ex{Vergleich} \ti{[f@Kgl\t*{aI}ç]}}
  \structureset{default n=}
  \drawCV{6}
  \wordstart
  \C{f}
...
\TODO{(how) are branching onsets parametrized?}
\TODO{are they recorded in the lexicon? If so, maybe no parametrization is needed
those languages that have it just have it, the others do not, but not necesarrily by an explicit rule?}
